...
namespace excels.Buffs.Debuffs
{
	...
	public class Marked : ModBuff
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Marked"); // Buff display name
			Description.SetDefault("You're being stalked"); // Buff description
			Main.debuff[Type] = true;  // Is it a debuff?
			Main.pvpBuff[Type] = true; // Players can give other players buffs, which are listed as pvpBuff
			Main.buffNoSave[Type] = true; // Causes this buff not to persist when exiting and rejoining the world
			//BuffID.Sets.LongerExpertDebuff[Type] = true; // If this buff is a debuff, setting this to true will make this buff last twice as long on players in expert mode
	...
	public class SpellCurse : ModBuff
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Spell Curse"); // Buff display name
			Description.SetDefault("Increased damage from all clerical attacks"); // Buff description
			Main.debuff[Type] = true;  // Is it a debuff?
			Main.pvpBuff[Type] = true; // Players can give other players buffs, which are listed as pvpBuff
			Main.buffNoSave[Type] = true; // Causes this buff not to persist when exiting and rejoining the world
										  //BuffID.Sets.LongerExpertDebuff[Type] = true; // If this buff is a debuff, setting this to true will make this buff last twice as long on players in expert mode
