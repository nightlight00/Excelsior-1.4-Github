...
namespace excels.Items.Armor.Priest
{
    ...
    public class PriestsAura : clericHealProj
    {
        ...
        public override void AI()
        {
            ...
            for (var i = 0; i < 6; i++)
            {
                Dust d = Dust.NewDustPerfect(player.Center + new Vector2(196).RotatedBy(MathHelper.ToRadians(Projectile.ai[0] + ((360 / 6) * i))), 204);
                d.noGravity = true;
                d.scale = 1.7f;
                d.velocity = Vector2.Zero;

                Dust d2 = Dust.NewDustPerfect(player.Center + new Vector2(196).RotatedBy(MathHelper.ToRadians(-Projectile.ai[0] + ((360 / 6) * i))), 204);
                d2.noGravity = true;
                d2.scale = 1.7f;
            ...
            if (++Projectile.ai[1] % 50 == 0) 
            {
                for (var i = 0; i < Main.maxPlayers; i++)
                {
                    ...
                    if (p2.statLife < p2.statLifeMax2 && Vector2.Distance(p2.Center, Projectile.Center) < 204)
                    {
