...
namespace excels.Items.WeaponHeal.Holyiest
{
    ...
    internal class GrandCross : ClericDamageItem
    {
        ...
        {
			Projectile p = Projectile.NewProjectileDirect(source, position, velocity.RotatedByRandom(MathHelper.ToRadians(8)), type, damage, knockback, player.whoAmI);
			p.ai[0] = 1;
			return false;
        }
	...
	internal class Prophecy : ClericDamageItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			for (var i = 0; i < 5; i++)
            {
				...
				Projectile p = Projectile.NewProjectileDirect(source, pos, velocity, type, damage, knockback, player.whoAmI);
			}
			return false;
		}
	}
	...
	public class ProphecyBolt : clericHealProj
    {
        ...
        {
			HealDistance(Main.LocalPlayer, Main.player[Projectile.owner], 10, 1, false);
